...
#===========================================================
# 2020-12-23 基于浪哥的自选股重新编写
# 2020-12-28 延长信号确认数据区间，每次取数据延迟的问题。
...
def handle_data(context,data):
    ...
    if not is_business_time(context):
        log.info('handle_data, 非买入策略盯盘时间,9:30~10:30')
        return
    ...
    if stocks_codes and order_cash > 0:
        ...
        for stock in stocks_codes:
            ...
            if stock in context.portfolio.positions:
                log.info('handle_data, 已经持仓的股票，跳过买入'+\
                    ', 股票代码：'+str(stock))
                continue
            # 获得昨天和前天的价格
...
def market_open_sell_all(context):
    ...
    for k,v in context.portfolio.positions.items():
        ...
        if not context._index_ma_is_down:
            ...
            pre_dt_at_1030 = context.previous_date.strftime('%Y-%m-%d') + " 10:30:00"
            #获得前一交易日10：30的价格
            prices_at_1030 = get_price(k, end_date=pre_dt_at_1030, \
                                    frequency='1m', fields=['close'], \
                                    skip_paused=True, fq='pre', count=1)
            
            #log.info('market_open_sell_all, 价格判断'+\
            #                ', 股票代码：'+str(k)+\
            #                ', 【'+str(pre_dt_at_1030)+'】价格:'+str(prices_at_1030['close'][0]) + \
            #                ', 昨日收盘价:'+str(prices['close'][0]) + \
            #                ', 昨日涨停价:'+str(prices['high_limit'][0]))
                            
            # 如果昨日涨停, 收盘价等于涨停价, 同时涨停打板时间在10：30之前
            if (prices['close'][0] - prices['high_limit'][0] >= 0) and \
               (prices_at_1030['close'][0] - prices['high_limit'][0] >= 0):
                log.info('market_open_sell_all, 昨日【涨停】,且在昨日【10:30】之前【涨停】, 早盘继续持有'+\
                            ', 股票代码：'+str(k)+\
                            ', 昨日【10:30】价格:'+str(prices_at_1030['close'][0]) + \
                            ', 昨日收盘价:'+str(prices['close'][0]) + \
                            ', 昨日涨停价:'+str(prices['high_limit'][0]))
                continue
        else:
...
def is_business_time(context):
    ''' 9:30～10：30 判断为买入盯盘时间段, 周期触发到31分''' 
    tim = context.current_dt.time()
    if tim.hour == 9:
        return True
    elif tim.hour == 10:
        return tim.minute <= 31
    else:
        return False
...
def filter_alorithm_3(context, df):
    tim = context.current_dt.time()
    #信号确认时间, 接收延迟10s以内的数据, 093000～093110
    xhqrsj_d = tim.hour * 10000 + (tim.minute - 1) * 100 + 0
    xhqrsj_u = tim.hour * 10000 + tim.minute * 100 + 10
    log.info('filter_alorithm_3, 信号确认时间段：【'+str(xhqrsj_d) + '】 ～ 【' + str(xhqrsj_u)+'】')
    #涨幅  不限制
    #深圳,创业板, 不包含科创板
    #只取1分钟
