...
Page({
  ...
  requestData: function () {
    ...
    if (_this.data.date != '' && _this.data.date != null) {
      url = 'https://api.limc.cn/api/Signal?ttl=0&count=50&_rnd=' + Math.random() + "&date=" + _this.data.date.replace(new RegExp('-', 'g'), '');
    } else {
      url = 'https://api.limc.cn/api/Signal?ttl=0&count=50&_rnd=' + Math.random();
    }
    // 请求数据
    wx.request({
      url: url,
  ...
  requestUISettingData: function () {
    const _this = this;
    // 拼接请求url
    const url = 'https://api.limc.cn/api/ui/settings.json?_rnd=' + Math.random();
    // 请求数据
    wx.request({
      url: url,
      data: {},
  ...
  requestUserData: function (res) {
    ...
    console.log(res)
    // 拼接请求url
    const url = 'https://api.limc.cn/api/UserInfo?_rnd=' + Math.random();
    // 请求数据
    wx.request({
      url: url,
      data: {
  ...
  requestSubscribe: function (res) {
    ...
    // 拼接请求url
    const url = 'https://api.limc.cn/api/Subscribe?oid=' + openid + '&_rnd=' + Math.random();
    // 请求数据
    wx.request({
      url: url,
      data: {},
