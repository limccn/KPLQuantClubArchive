...
def step_jump_to_trade_tab():
    ...
    if btn_trade.exists():
        ...
        ###poco("android.widget.FrameLayout").child("android.widget.LinearLayout").offspring("com.hexin.plat.android:id/tab_a").click()
        
        #poco("com.hexin.plat.android:id/tab_a").click()
        #sleep(0.5/_SPEED_) 
    else:
...
def step_select_menu_buy():
    ...
    if navibuton.exists() and navibuton.attr("selected"):
        pass
        #sleep(0.5/_SPEED_)
    else:
        raise Exception("Navibar Not in buy tab")
...
def step_select_menu_sell():
    ...
    if navibuton.exists() and navibuton.attr("selected"):
        ...
        raise Exception("Navibar Not in sell tab")
...
def step_select_menu_cancel():
    ...
    if navibuton.exists() and navibuton.attr("selected"):
        ...
        raise Exception("Navibar Not in Cancel tab")
...
def step_input_stock_sell_price(price, maket_price=False,limit_price=False,cancel_ifis_upper=False):
    order_price = price

    #检查涨跌停板 
    if cancel_ifis_upper:
        str_current = poco("com.hexin.plat.android:id/layout_stockprice").child("com.hexin.plat.android:id/stockprice").offspring("android.widget.EditText").get_text()
        str_limit = poco("com.hexin.plat.android:id/zhangting_layout").child("com.hexin.plat.android:id/zhangtingprice").get_text()
    
        #涨停板下一分
        if float(str_limit) - float(str_current) <=0.01:
            raise Exception("Current Price is upper limit, cancel order")
        else:
            pass
    else:
        pass
...
def check_login_state(name, account):
    call_result = False
    try:
        step_startup_app()
        step_clear_popup()
        call_result = step_check_account_login(name, account)
    except Exception as e:
        log(e)
        call_result = False
    finally:
...
def order_sell(code,price=0.,number=0 ,percent=0.,maket_price=False,limit_price=False,cancel_ifis_upper=False):
    call_result = False
    try:
        step_reload_app_from_init()
        ...
        order_price = step_input_stock_sell_price(price, maket_price, limit_price,cancel_ifis_upper)
