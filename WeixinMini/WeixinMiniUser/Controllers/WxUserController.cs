...
namespace WeixinMiniUser.Controllers
{
    public class WxUserController : ApiController
    {
        ...
        public Message<UserInfo> Get(string cd)
        {
            ...
            try
            {
                using (HttpWebResponse resp = WeixinCommon.HttpHelper.CreateGetHttpResponse(url))
                {
        ...
        public Message<WXUserInfo> Post([FromBody] WXUserInfoReq wxuser)
        {
            ...
            string userInfoDes = WeixinCommon.Security.WXAESDecrypt(wxuser.iv, wxuser.sk, wxuser.ed);
            if(userInfoDes == null)
            {
                result.code = 400;
