...
namespace KaiPanLa
{
    public partial class FrmAnalyse : Form
    {
        ...
        private double dRateFrom = 2.0;
        private double dRateTo = 6.0;
        private double dSJLTPFrom = 15;
        private double dSJLTPTo = 1000;
        private double dTL = 320;
        private double dQD = 32;
        ...
        {
            this.Location = new Point(size.Width / 2 - 10, 0);
            this.Width = size.Width / 2 - 10;
            this.Height = size.Height / 2 - 100;
        }

        public void setupStockRankingData(DataTable dt, DateTime updateTime)
        {
            if (dt == null)
            {
                return;
        ...
        public void queryDatatable()
        {
            ...
            //先清空
            DataTableToEntity<StockAnalyseListItemEntity> util = new DataTableToEntity<StockAnalyseListItemEntity>();
            List<StockAnalyseListItemEntity> listClear = new List<StockAnalyseListItemEntity>();
            ...
            List<StockAnalyseListItemEntity> list = new List<StockAnalyseListItemEntity>();
            int index = 0;
            foreach (var row in query)
            {
                var newEntity = new StockAnalyseListItemEntity
                {
                    _Number = ++index,
            ...
            if (dtEntity != null)
            {
                frmMain.handleStockAnalyseData(dtEntity.Copy(), this.updateTime);
            }
            ...
            if (query2.Count() > 0)
            {
                ...
                //传值给Mdi窗体
                frmMain.handleFilterdStockAnalyseData(filterd, this.updateTime);
            ...
            // 最后数据入库
            if (Common.getShouldWriteDataToDB() && Common.getShouldWriteLatestAnalyseToMemoryTable())
            {
        ...
        private void latestAnalyseTableToDB(DataTable dt)
        {
            ...
            using (SqlConnection destinationConnection = new SqlConnection(connectionString))
            {
                ...
                string stockLatestAnalyseTableName = Common.GetAppSettingByKey("StockLatestAnalyseTableName");//要插入的表的表名
        ...
        private void filterdAnalyseTableToDB(DataTable dt)
        {
            ...
            using (SqlConnection destinationConnection = new SqlConnection(connectionString))
            {
                ...
                string StockAnalyseTableName = Common.GetAppSettingByKey("StockAnalyseTableName");//要插入的表的表名
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(destinationConnection))
                {
                    try
                    {
                        ...
                        bulkCopy.DestinationTableName = StockAnalyseTableName;//要插入的表的表名
                        bulkCopy.BatchSize = dt.Rows.Count;
                        ...
                        bulkCopy.WriteToServer(dt);
                        logger.Info("DataTable写入DB完成,表名：" + StockAnalyseTableName);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(ex.Message);
                        logger.Warn("DataTable写入DB失败,表名：" + StockAnalyseTableName, ex);
                    }
                    finally
                    {
                        if (destinationConnection.State == ConnectionState.Open)
        ...
        {
            FrmAnalyseFilterCond frmFilterCondition = new FrmAnalyseFilterCond();
