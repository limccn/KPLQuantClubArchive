...
namespace KaiPanLaWeb.Daos
{
    public class PortfolioDao
    {
        ...
        public List<Portfolio> QueryPortfolios(DateTime date, Int64 ttl, Int64 count)
        {
            DataTable dtInDb = QueryDataTableFromDB(date, ttl, count);
            DataTableToEntity<Portfolio> util = new DataTableToEntity<Portfolio>();
            List<Portfolio> result = util.FillModel(dtInDb);
        ...
        private DataTable QueryDataTableFromDB(DateTime date, Int64 ttl, Int64 count)
        {
            ...
            using (SqlConnection destinationConnection = new SqlConnection(connectionString))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.Warn("Portfolio数据查询失败", ex);
                }
                finally
                {
